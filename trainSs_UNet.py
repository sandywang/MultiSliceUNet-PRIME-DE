...
import argparse
import numpy as np
from testSs_UNet import *

NoneType=type(None)
# Argument
parser = argparse.ArgumentParser(description='Train')
parser.add_argument('-tt1w', '--train_t1w', type=str, required=True, help='Train T1w Directory')
parser.add_argument('-tmsk', '--train_msk', type=str, required=True, help='Train Mask Directory')
...
model=UNet2d(dim_in=3, num_conv_block=5, kernel_root=16)
if isinstance(args.init_model, str):
    if not os.path.exists(args.init_model):
        print("Invalid Init Model")
        sys.exit(2)
...
if use_validate:
    ...
    dice_dict=predict_volumes(valid_model, rimg_in=None, cimg_in=args.validate_t1w, bmsk_in=args.validate_msk, 
        rescale_dim=rescale_dim, num_slice=3, save_nii=False, save_dice=True)
...
for epoch in range(0, args.num_epoch):
    ...
    for i, (cimg, bmsk) in enumerate(volume_loader):
        block_dataset=BlockDataset(rimg=cimg, bfld=None, bmsk=bmsk, num_slice=3, rescale_dim=rescale_dim)
